...
class TestInterfaceCase(ParametrizedTestCase):
   ...
   def test_login_normal(self):
       # 根据被测接口的实际情况，合理的添加HTTP头
        ...
        try:
           ...
           # 断言
            self.assertEqual(response['code'], 0, msg=u'返回code不等于0')
            self.assertEqual(response['msg'], u'登录成功', msg=u'登录失败')
            self.assertEqual(response['data']['sex'], 2, msg=u'sex错误')
            self.assertEqual(response['data']['cityId'], None, msg=u'cityId错误')
            self.assertEqual(response['data']['nikeName'], None, msg=u'nikeName错误')
            self.assertEqual(response['data']['cityName'], None, msg=u'cityName错误')
            self.assertEqual(response['data']['userId'], user_id, msg=u'userId错误')  #2910057590
            self.assertEqual(response['data']['cityName'], None, msg=u'cityName错误')
            self.assertEqual(response['data']['payPasswordFlag'], 1, msg=u'payPasswordFlag错误')
            self.assertEqual(response['data']['imgSmall'], None, msg=u'imgSmall错误')
            self.assertEqual(response['data']['imgBig'], None, msg=u'imgBig错误')
   ...
   def test_chpasswd_normal(self):
       ...
       try:
           self.assertEqual(response['code'], 0, msg=u'返回code不等于0')
           self.assertEqual(response['msg'],u'支付密码修改成功', msg=u'修改支付密码失败')
           self.assertEqual(response['data'],None, msg=u'data不为N')
           self.test_data.result = 'Pass'
       except AssertionError as e:
           print('%s' % e)
           self.test_data.result = 'Fail'
